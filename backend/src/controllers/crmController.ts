...
// Helper: Simple email sending function (uses platform settings)
const sendEmail = async (options: { to: string; subject: string; html: string }) => {
    // Get platform settings for email
    const settings = await prisma.platformSettings.findFirst();

    if (!settings?.azureEmailEnabled) {
        console.warn('Email service not configured');
        return false;
    }

    // TODO: Implement actual email sending using Azure or SMTP
    // For now, just log
...
export const calculateLeadScore = async (req: Request, res: Response) => {
    try {
        ...
        const engagementScore = Math.min(
            activities.length * 5 + emails.filter((e: any) => e.openCount > 0).length * 10,
            40
        );

        const emailOpens = emails.reduce((sum: number, e: any) => sum + e.openCount, 0);
        const emailClicks = emails.reduce((sum: number, e: any) => sum + e.clickCount, 0);
